...
class CLogger 
{
	public:
		...
		CLogger(const char* fileName, const char* basePath, unsigned int numberOfBackups = DEFAULT_MAX_FILES, unsigned int maxFileSize = DEFAULT_MAX_FILE_LEN, LOG_PRIORITY priority = TRACE);
		virtual ~CLogger();
	...
	private:
		...
		char* getPriorityName( LOG_PRIORITY priority);
