...
void CLogger::logAscii(const char* fileName, const char* functionName, LOG_PRIORITY priority, const char* format, ...)
{
	...
	if (priority >= m_priority)
	{
		...
		if (fp)
		{
			...
			strftime(dateHour, sizeof (dateHour), "%d-%m-%Y %H:%M:%S", localtime( &(now.time) ));
			sprintf(str, "%s.%03d %s %s %s ", dateHour, now.millitm, getPriorityName( priority), fileName, functionName);
...
char* CLogger::getPriorityName( LOG_PRIORITY priority)
{
	switch( priority)
	{
		case 0: strcpy( priorityName, "TRACE"); break;
		case 1: strcpy( priorityName, "WARN"); break;
		case 2: strcpy( priorityName, "ERROR"); break;
		case 3: strcpy( priorityName, "FAIL"); break;
		default: strcpy( priorityName, "UNDEF"); break;
	}
	return priorityName;
}
