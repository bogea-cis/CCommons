...
#ifndef COMMUNICATION_CSERIAL_H
#define COMMUNICATION_CSERIAL_H
...
class CSerial
{
	...
	public:
		...
		virtual LONG Open (LPCTSTR lpszDevice, DWORD dwInQueue = 0, DWORD dwOutQueue = 0, bool fOverlapped = SERIAL_DEFAULT_OVERLAPPED, bool buffering = true);
		...
		virtual LONG SetupReadTimeouts (int interval, int total, int multiplier);
		...
		LONG PurgeRx ();
		// Purge all buffers
		LONG Purge (unsigned long flags = PURGE_TXCLEAR | PURGE_RXCLEAR | PURGE_TXABORT | PURGE_RXABORT);
...
#endif//HST_COMMUNICATION_CSERIAL_H
